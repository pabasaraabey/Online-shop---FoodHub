...
const ProductDetails = () => {
    ...
    const { id } = useParams();
    const [relatedProducts, setRelatedeProducts] = useState([]);
    ...
    useEffect(() => {
        if (product) {
            ...
            const related = productscopy.filter(
                (item) => item.category === product.category && item._id !== product._id
            );
            setRelatedeProducts(related.slice(0, 5));
        }
    }, [product]);
                ...
                <div className="text-sm w-full md:w-1/2">
                    ...
                    <div className="flex items-center gap-0.5 mt-1">
                        {Array(5).fill('').map((_, i) => (
                            <img key={i} src={i < 4 ? assets.star_icon : assets.star_dull_icon} alt={`star-${i}`} className="md:w-4 w-3.5" />
                        ))}
